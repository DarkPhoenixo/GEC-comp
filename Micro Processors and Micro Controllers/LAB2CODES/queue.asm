section .data
    ...
    enqueue_prompt db "Enter value > ", 0
    enqueue_prompt_len equ $ - enqueue_prompt
    
    enqueue_success db " Value inserted to queue.", 10, 0
    enqueue_success_len equ $ - enqueue_success
    
    queue_full db " Queue is full!", 10, 0
    queue_full_len equ $ - queue_full
    
    dequeue_success db "Deleted > ", 0
    dequeue_success_len equ $ - dequeue_success
    
    queue_empty db "Queue is empty! ", 10, 0
    queue_empty_len equ $ - queue_empty
    
    display_empty db " Queue is empty.", 10, 0
    display_empty_len equ $ - display_empty
    
    display_header db "Queue contents [Front → Rear]: ", 0
    display_header_len equ $ - display_header
    
    newline db 10, 0
    space db " ", 0
    
    invalid_choice db "Invalid selection. Try again.", 10, 0
    invalid_choice_len equ $ - invalid_choice
    
    exit_msg db "Terminating program...", 10, 0
